...
if exist "%RCEDIT%" (
  "%RCEDIT%" "%OUT%\_base.exe" --set-icon "%ICO%" --set-version-string ProductName "Bridge" --set-version-string FileDescription "Bridge" --set-version-string CompanyName "Shaltiel Enterprises" --set-version-string LegalCopyright "Copyright 2026 Shaltiel Enterprises - developed by WhiteNo1s3" --set-version-string OriginalFilename "Bridge.exe"
) else (
